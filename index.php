...
async function showNotes(){const r=await fetch('?action=api_notes&_='+Date.now());const n=await r.json();let h='<div style=max-height:400px;overflow-y:auto><table style=width:100%;border-collapse:collapse;font-size:13px><tr style=background:var(--lb)><th style=\"padding:8px;text-align:left\">ID</th><th style=padding:8px>Preview</th><th style=padding:8px>Updated</th><th style=padding:8px>Action</th></tr>';for(const x of n){const d=new Date(x.updated_at).toLocaleString('zh-CN'),c=x.content?x.content.substring(0,30)+(x.content.length>30?'...':''):'<em style=opacity:0.5>Empty</em>',is=x.slug===SLUG;h+='<tr style=\"background:'+(is?'var(--ac)':'transparent')+';color:'+(is?'#fff':'var(--tc)')+'\"><td style=\"padding:8px;font-family:monospace\">'+x.slug+'</td><td style=padding:8px>'+c+'</td><td style=\"padding:8px;font-size:12px\">'+d+'</td><td style=padding:8px><a href=\"?note='+x.slug+'\" style=\"color:'+(is?'#fff':'var(--ac)')+'\">'+(is?'Current':'Open')+'</a></td></tr>';}h+='</table></div>';document.getElementById('nlist').innerHTML=n.length?h:'<p style=text-align:center;padding:32px;color:var(--sc)>No notes</p>';document.getElementById('lmo').classList.add('on');}
